...
class DecisionTree:
    ...
    def _entropy(self, y):
        ...
        for count in counts:
            if count > 0:
                entropy -= (count/len(y)) * np.log(count/len(y))
        # ====================================================
    ...
    def _find_split(self, X, y, H_data):
        ...
        for ii in range(unique_values.shape[0] - 1):
            ...
            current_split_value = (unique_values[ii] + unique_values[ii+1])/2
            current_split_index = ii
            H_left = self._entropy(y[0:first_idxes[ii+1]])
            H_right = self._entropy(y[first_idxes[ii + 1]:])
            H_cond = (first_idxes[ii+1]/len(y))*H_left + (1 - first_idxes[ii+1]/len(y))*H_right
            current_information_gain = H_data - H_cond
            if self.debug:
    ...
    def _build_tree(self, X, y, level):
        ...
        if N < self.min_leaf_data or H_data < self.min_entropy or self.max_depth <= level:
            ...
            if self.debug:
                print(f"11111Num Samples: {N}, Entropy: {H_data}, Depth: {level}, Probs: {probs.T}")
